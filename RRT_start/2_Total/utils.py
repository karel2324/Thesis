...
def get_base_data_dir() -> Path:
    ...
    Returns:
        ...
        - Colab: /content/drive/MyDrive/Thesis/Data
        - Local: c:/Users/karel/Desktop/data/Thesis/Data
    """
    if is_colab():
        # Google Colab with Drive mounted
        colab_path = Path("/content/drive/MyDrive/Thesis/Data")
        if colab_path.exists():
            return colab_path
        # Fallback: check if drive is mounted but different structure
        alt_path = Path("/content/drive/My Drive/Thesis/Data")
        if alt_path.exists():
            return alt_path
        raise FileNotFoundError(
            "Google Drive not mounted or data not found. "
            "Run: drive.mount('/content/drive') and upload data to MyDrive/Thesis/Data/"
        )
    else:
        # Local Windows environment
        return Path(r"c:\Users\karel\Desktop\data\Thesis\Data")
...
def get_data_paths(config: dict) -> dict:
    ...
    # Base data directory (auto-detect environment)
