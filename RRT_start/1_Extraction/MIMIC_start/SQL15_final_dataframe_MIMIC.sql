...
CREATE OR REPLACE TABLE `windy-forge-475207-e3.derived_mimic.grid_master_all_features` AS
WITH
/* ===============================================================
...
sofa_components AS (
  ...
  FROM grid g
  LEFT JOIN `windy-forge-475207-e3.derived_mimic.sofa_cardio_24h` sc
    USING (subject_id, hadm_id, stay_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived_mimic.sofa_renal_24h` sr
    USING (subject_id, hadm_id, stay_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived_mimic.sofa_resp_24h` ss
    USING (subject_id, hadm_id, stay_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived_mimic.sofa_neuro_24h` sn
    USING (subject_id, hadm_id, stay_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived_mimic.sofa_coag_24h` sco
    USING (subject_id, hadm_id, stay_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived_mimic.sofa_liver_24h` sl
    USING (subject_id, hadm_id, stay_id, grid_ts)
),
...
last_var_wide AS (
  ...
  FROM `windy-forge-475207-e3.derived_mimic.grid_last_var_long`
...
SELECT
  ...
  /* ---- renal trends ---- */
  rt.creat_rel_inc_48h,
  IF(rt.creat_rel_inc_48h IS NULL, 1, 0) AS creat_rel_inc_48h_missing,

  rt.urea_rel_inc_48h,
  IF(rt.urea_rel_inc_48h IS NULL, 1, 0) AS urea_rel_inc_48h_missing,
...
FROM grid g
LEFT JOIN `windy-forge-475207-e3.derived_mimic.cohort_static_variables` cs
  USING (subject_id, hadm_id, stay_id)
LEFT JOIN sofa_all s
  USING (subject_id, hadm_id, stay_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived_mimic.grid_urine_output` uo
  USING (subject_id, hadm_id, stay_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived_mimic.grid_renal_trends` rt
  USING (subject_id, hadm_id, stay_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived_mimic.cohort_vasopressors_grid` vp
  USING (subject_id, hadm_id, stay_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived_mimic.cohort_mechanical_ventilation_grid` mv
  USING (subject_id, hadm_id, stay_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived_mimic.grid_fluid_balance` fb
  USING (subject_id, hadm_id, stay_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived_mimic.cohort_gcs_grid` gcs
  USING (subject_id, hadm_id, stay_id, grid_ts)
