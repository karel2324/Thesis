CREATE OR REPLACE TABLE
  `windy-forge-475207-e3.derived.grid_master_all_features` AS
WITH
/* ===============================================================
...
sofa_components AS (
  ...
  FROM grid g
  LEFT JOIN `windy-forge-475207-e3.derived.sofa_cardio_24h` sc
    USING (person_id, visit_occurrence_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived.sofa_renal_24h` sr
    USING (person_id, visit_occurrence_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived.sofa_resp_24h` ss
    USING (person_id, visit_occurrence_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived.sofa_neuro_24h` sn
    USING (person_id, visit_occurrence_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived.sofa_coag_24h` sco
    USING (person_id, visit_occurrence_id, grid_ts)
  LEFT JOIN `windy-forge-475207-e3.derived.sofa_liver_24h` sl
    USING (person_id, visit_occurrence_id, grid_ts)
),
...
SELECT
  ...
  cs.weight_kg_first,
  cs.weight_used_kg_person,
  cs.weight_source_person,
  cs.admit_dt,
  cs.discharge_dt,
  ...
  /* weight is static (first measurement per person) */
  IF(cs.weight_kg_first IS NULL, 1, 0) AS weight_kg_first_missing

FROM grid g
LEFT JOIN `windy-forge-475207-e3.derived.cohort_static_variables` cs
  USING (person_id, visit_occurrence_id)
LEFT JOIN sofa_all s
  USING (person_id, visit_occurrence_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived.grid_urine_output` uo
  USING (person_id, visit_occurrence_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived.grid_renal_trends` rt
  USING (person_id, visit_occurrence_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived.cohort_vasopressors_grid` vp
  USING (person_id, visit_occurrence_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived.cohort_mechanical_ventilation_grid` mv
  USING (person_id, visit_occurrence_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived.grid_fluid_balance` fb
  USING (person_id, visit_occurrence_id, grid_ts)
LEFT JOIN `windy-forge-475207-e3.derived.cohort_gcs_grid` gcs
  USING (person_id, visit_occurrence_id, grid_ts)
