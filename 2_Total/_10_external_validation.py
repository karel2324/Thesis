...
def run_external_validation(config: dict, all_paths: dict):
    ...
    if ext_cfg['algorithms']['bc']:
        ...
        if algo_vs_bc_results:
            ...
            if ext_cfg['output']['save_plots']:
                # Rename columns to match create_plots format
                ...
                create_plots_algo_vs_bc(results_df=plot_df, output_dir=output_dir, source_db=source_db, target_db=target_db)
...
def create_plots_algo_vs_bc(results_df: pd.DataFrame, output_dir: Path, source_db: str, target_db: str):
    """
    Create comparison plots for RL algorithms vs BC (external validation).
    ...
    for split in results_df['split'].unique():
        ...
        # Create figure with 2x3 subplots
        fig, axes = plt.subplots(nrows=2, ncols=3, figsize=(15, 10))
        fig.suptitle(f'RL Algorithms vs BC: {source_db.upper()} → {target_db.upper()} ({split.upper()} Split)',
                     fontsize=14, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f'external_validation_algo_vs_bc_{split}.png', dpi=150)
        plt.close()
        print(f"  Saved: external_validation_algo_vs_bc_{split}.png")
