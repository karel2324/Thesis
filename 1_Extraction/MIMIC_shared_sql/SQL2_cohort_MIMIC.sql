...
chosen_index AS (
  ...
  SELECT k3.stay_id, k3.t0
  FROM kdigo3_index k3
  CROSS JOIN cfg
  WHERE cfg.inclusion_default = 'kdigo3'
),
...
/* ===============================================================
   5) TERMINAL STATE (death, discharge, CRRT start, window_end)
   =============================================================== */
terminal AS (
  SELECT
    ci.stay_id,
    ...
    -- Window end = t0 + obs_days (observation period cap)
    TIMESTAMP_ADD(ci.t0, INTERVAL cfg.obs_days DAY) AS window_end,

    -- Terminal timestamp = earliest of death/discharge/CRRT/window_end
    LEAST(
      ...
      TIMESTAMP_ADD(ci.t0, INTERVAL cfg.obs_days DAY)  -- window_end cap
    ) AS terminal_ts,

    -- Terminal event type (order: rrt_start > death > discharge > window_end)
    CASE
      WHEN cf.crrt_start_dt IS NOT NULL
           ...
           AND TIMESTAMP(cf.crrt_start_dt) <= TIMESTAMP_ADD(ci.t0, INTERVAL cfg.obs_days DAY)
      THEN 'rrt_start'
