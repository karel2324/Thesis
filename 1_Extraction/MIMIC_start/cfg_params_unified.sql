/*
================================================================================
UNIFIED CONFIGURATION - MIMIC-IV & AUMCdb
================================================================================

Dit bestand bevat alle configuratieparameters voor beide databases.
Gebruik: Run dit script eerst om derived_mimic.cfg_params aan te maken.

Structuur:
  - global_params: Tijdsparameters (obs_days, grid_step, etc.)
  - inclusion/exclusion: Cohort criteria
  - var_defs: Variable definities met IDs per database + units + conversie
...
CREATE OR REPLACE TABLE derived_mimic.cfg_params AS
SELECT
  ...
  -- ============================================================================
  'kdigo2' AS inclusion_default,

  -- ============================================================================
  -- 3. EXCLUSION CRITERIA
  -- ============================================================================
  12 AS min_icu_stay_hours,
  ...
  [
    -- RENAL / METABOLIC
    STRUCT(
      'creat' AS var, 'renal' AS category, 48 AS lb_h,
      0 AS min_clin, 3000 AS max_clin, 0 AS min_stat, 3000 AS max_stat,
      [3020564] AS aumcdb_ids, 'μmol/L' AS aumcdb_unit,
    ...
    STRUCT(
      'temperature', 'vital', 48,
      30, 45, 30, 45,
      [21490586, 3022060, 3025085], '°C',
      [223761], '°F',
  ...
  STRUCT(
    ...
    CAST(NULL AS ARRAY<INT64>) AS mimic_ids  -- Use derived.ventilation
  ) AS vent_ids,

  -- Bloodflow (CRRT)
  STRUCT(
    [1988610] AS aumcdb_ids,
    CAST(NULL AS ARRAY<INT64>) AS mimic_ids  -- Use derived.crrt
  ) AS bloodflow_ids,
