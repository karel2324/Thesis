CREATE OR REPLACE TABLE `windy-forge-475207-e3.derived.cohort_static_variables` AS
WITH
cohort AS (
  SELECT
    person_id,
  ...
  FROM `windy-forge-475207-e3.derived.cohort_index`
...
dem AS (
  SELECT
    ...
    gender,
    birth_date
  FROM `windy-forge-475207-e3.derived.demographics`
),
...
vt AS (
  SELECT
    ...
    admit_dt,
    discharge_dt
  FROM `windy-forge-475207-e3.derived.visit_times`
),
